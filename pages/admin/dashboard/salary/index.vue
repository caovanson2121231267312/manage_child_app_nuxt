...
export default {
    ...
    methods: {
        ...
        async load_data() {
            await api.get(`chi-luong/danh-sach?tuKhoa=${this.tuKhoa}&dien_thoai=&leader_kd_id=${this.leader_kd_id}&dia_chi=&dich_vu_id=${this.dich_vu_id}&page=${this.current_page}&limit=1000000&sort=1&thang=${(this.$route.query.t ?? this.thang_id) +'/'+ (new Date()).getFullYear()}`, {
        ...
        async export_excel() {
            ...
            await api.get(`chi-luong/excel-danh-sach-phieu-luong?tuKhoa=${this.tuKhoa}&dien_thoai=&leader_kd_id=${this.leader_kd_id}&dia_chi=&dich_vu_id=${this.dich_vu_id}&thang=${this.thang_id +'/'+ (new Date()).getFullYear()}&page=1&limit=&sort=`, {
                'Content-Type': 'multipart/form-data',
                Authorization: 'Bearer ' + this.token
            }).then(res => {
                Swal.close()
